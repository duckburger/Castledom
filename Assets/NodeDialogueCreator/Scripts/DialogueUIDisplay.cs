...
#region Processing Conversation Asset
    ...
    IEnumerator RollOutLine(string line)
    {
        ...
        else if (currentNode.GetConnectedNPCLines().Count > 0)
        {
            ...
            DisplayLine(currentNode.GetConnectedNPCLines()[0]);
        }
    }


    void ShowReplies(List<PlayerDialogueNode> replies)
    {
        if (!responsePrefab)
        {
        ...
        for (int i = 0; i < replies.Count; i++)
        {
            ...
            replyText.text = replies[i].dialogueLine;
            NPCDialogueNode connectedNPCNode = replies[i].outgoingTransitions[0].endNPCNode as NPCDialogueNode;                           

            replyButton.onClick.AddListener(() => 
            {
                AnimateRepliesOut();
                currentNode = connectedNPCNode;
                DisplayLine(currentNode);
            });
        }
